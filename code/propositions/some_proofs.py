...
def prove_and_commutativity() -> Proof:
    ...
    f0 = Formula.parse('q')
    f1 = Formula.parse('p')
    f2 = Formula.parse('(p&q)')
    f3 = Formula.parse('(q&p)')
    l0 = Proof.Line(f2)
    l1 = Proof.Line(f0, AE1_RULE, [0])
    l2 = Proof.Line(f1, AE2_RULE, [0])
    l3 = Proof.Line(f3, A_RULE, [1, 2])
    lines = [l0, l1, l2, l3]
...
def prove_I0() -> Proof:
    ...
    f2 = Formula.parse('(p->(p->p))')
    l2 = Proof.Line(f2, I1, [])

    f3 = Formula.parse('((p->(p->p))->(p->p))')
    l3 = Proof.Line(f3, MP, [0, 1])

    f4 = Formula.parse('(p->p)')
    l4 = Proof.Line(f4, MP, [2, 3])
